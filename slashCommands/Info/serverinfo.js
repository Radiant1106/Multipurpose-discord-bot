  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_Radiant_6966 => this_Code_is_by_Radiant_6966).length > maxLen) {
          const len = arr.map(this_Code_is_by_Radiant_6966 => this_Code_is_by_Radiant_6966).length - maxLen;
          arr = arr.map(this_Code_is_by_Radiant_6966 => this_Code_is_by_Radiant_6966).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .setAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
          dynamic: true
        }), "https://discord.com/api/oauth2/authorize?client_id=948701351309897748&permissions=8&scope=bot%20applications.commands")
