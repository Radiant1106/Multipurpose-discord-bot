    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor({ name: "Welcome Setup", url: "https://discord.cyberneticsdev.tk", iconURL: "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png" })
          //.setAuthor('Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.cyberneticsdev.tk')
        ...
        switch (optionhandletype) {
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata) {
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`: {
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({ embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:tick:1005792950099398676> **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**"}` })
                        });
                        //Menu Collections
          ...
          case "Direct Welcome Messages": {
            ...
            async function second_layer() {
              ...
              let MenuEmbed = new MessageEmbed()
                .setColor(es.color)
                .setAuthor('DM - Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.cyberneticsdev.tk')
                .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
              //send the menu msg
              let menumsg = await message.reply({ embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)] })
              //Create the collector
          ...
          case `Test Welcome`: {
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:Like:1005819252437164062> **X Invites**\n[<:joines:992230927897997342> X Joins | <:leavessss:993120411942400051> X Leaves | <a:no:1005793164046630912> X Fakes]`
            if (welcome) {
