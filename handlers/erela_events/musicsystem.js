...
module.exports = client => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player.queue.current))
            return interaction?.reply({content: "<a:no:997458422821818449> Nothing Playing yet", ephemeral: true})
    ...
    client.on("messageCreate", async message => {
        ...
        if (!channel) return message.reply("<a:no:997458422821818449> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<a:no:997458422821818449> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setDescription(`> *I support <:youtube:997460775742488708> Youtube, <:Spotify:997461107197362226> Spotify, <:soundcloud:825095625884434462> Soundcloud and direct MP3 Links!*`)
    ]
