  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setTitle(`<a:Join_vc:997469848810553394> Joined a New Server`)
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is Radiant, and the Bot is in not a Cybernetics Development, Public Bot, then dont send information!
      if(owner == "552060563602276362"){
        let CyberneticsGuild = client.guilds.cache.get("914099997639868417");
        if(CyberneticsGuild && !CyberneticsGuild.me.roles.cache.has("952581433162096661")){
          continue; 
        }
      }
  ...
  client.on("guildDelete", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("RED")
      .setTitle(`<:leavessss:993120411942400051> Left a Server`)
      .addField("Guild Info", `>>> \`\`\`${guild.name} (${guild.id})\`\`\``)
      .addField("Owner Info", `>>> \`\`\`${theowner ? `${theowner.tag} (${theowner.id})` : `${theowner} (${guild.ownerId})`}\`\`\``)
      .addField("Member Count", `>>> \`\`\`${guild.memberCount}\`\`\``)
      .addField("Servers Bot is in", `>>> \`\`\`${client.guilds.cache.size}\`\`\``)
